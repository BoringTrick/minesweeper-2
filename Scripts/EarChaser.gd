...
# can be changed by gamemode script
@export var cooldownTime = 3.25

# the chaser's name!
@export var chaserName = "Dr. Ear"
...
func moveToPos(_globalMousePos, _clickedTile, context):
	# make sure dr ear isnt busy
	if gameManager.gameState == "playing" and isBusy == false and (context == "tileReveal" or context == "tileFlagged"):
		isBusy = true
		chaserLine.points[1] = self.get_global_mouse_position()
		crosshair.global_position = self.get_global_mouse_position()
		chaserLine.global_position = Vector2(0, 0)
		chaserLine.visible = true
		crosshair.visible = true
		# very simple chase to the clicked position logic
		var tweenSpeed = ((self.global_position - self.get_global_mouse_position()).length()) / 950
		gameManager.chaserMoved.emit(self, self.global_position, tweenSpeed)
		var tweenAttack = get_tree().create_tween().set_parallel(true)
		tweenAttack.tween_property(self, "global_position", self.get_global_mouse_position(), tweenSpeed)#.set_trans(Tween.TRANS_BOUNCE)
		tweenAttack.tween_property(crosshair, "global_position", self.get_global_mouse_position(), tweenSpeed)#.set_trans(Tween.TRANS_BOUNCE)
